    ...
    def handle(self, *args, **options):
        ...
        for user_profile in user_applied_data:
            ...
            if (len(user_data) == 10000):
                keys = user_data[key_pos].keys()
                print keys

                with open('/home/ubuntu/vms2/user_child_Data_' + str(randint(1, 10000)) + '.csv', 'w+') as f:
            ...
            if (user_persistent_form_data is None or user_persistent_form_data.count() == 0):
                ...
                user_dict["No. of Children"]="No info available"
                ...
                if "Children" in user_persistent_form_data[0].data.keys():
                    ...
                    for child in user_persistent_form_data[0].data["Children"]:
                        ...
                        if ("Name" in child.keys()):
                            user_dict["Child Name " + str(count_c)] = child["Name"]
                        else:
                            user_dict["Child Name" + str(count_c)] = "No Info Available"

                        if ("Date of Birth" in child.keys()):
                            user_dict["Child DOB " + str(count_c)] = child["Date of Birth"]
                        else:
                            user_dict["Child DOB" + str(count_c)] = "No Info Available"

                        if ("Gender" in child.keys()):
                            user_dict["Gender" + str(count_c)] = child["Gender"]
                        else:
                            user_dict["Gender" + str(count_c)] = "No Info Available"

                        if ("School Name" in child.keys()):
                            user_dict["School Name" + str(count_c)] = child["School Name"]

                        else:
                            user_dict["School Name" + str(count_c)] = "No Info Available"
                    user_dict["No. of Children"]=str(count_c)
                    
                    if count_c>max_count:
                        max_count=count_c
                        key_pos=len(user_data)
