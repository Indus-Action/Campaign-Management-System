...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
       ...
       # tasks = Task.objects.filter(task_type__task_type='Retention Survey').filter(status__status='Call Done').exclude(beneficiary__tags__in=[Tag.objects.get(tag='Data Export Progress')])[:1500]
        tasks=Task.objects.filter(task_type__task_type='CC Admitted Survey 2').filter(status__status='Call Done')

        print "The correct form data is "+str(tasks.count())

        print "Extracting the first piece of information"

        for task in tasks:
            data={}
            form=task.form_data
            user=task.beneficiary
            persistent_form=PersistentFormData.objects.get(beneficiary=user)
            ...
            if (persistent_form is not None):
                form_data=persistent_form.data
                print "PArsing persistent data"
                for key,value in form_data.iteritems():
        ...
        with open('/home/ubuntu/vms2/admitted2.csv', 'w+') as f:
